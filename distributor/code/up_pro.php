...
{	
	...
	{
		//mobile no update
		$result=pg_query($conn,"select m_no from distributor_detail where m_no=$data AND did!=$did") or pg_error($conn);
		...
		{
			echo 'True';
			pg_query($conn,"update distributor_detail set m_no=$data where did=$did");
		}
	}
	else if($_POST['sub']=='e')
	{
		//email update	
		$result=pg_query($conn,"select e_id from distributor_detail where e_id='$data' AND did!=$did") or pg_error($conn);
		...
		{
			echo 'True';
			pg_query($conn,"update distributor_detail set e_id='$data' where did=$did");
		}
	}
	else if($_POST['sub']=='p')
	{
		//password update
		echo 'True';
		pg_query($conn,"update distributor_detail set pwd='$data' where did=$did");	
	}
}
else
{
