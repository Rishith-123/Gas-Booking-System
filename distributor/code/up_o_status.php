...
if(isset($_GET['oid']) and isset($_SESSION['did']))
{
	...
	$sval=$_GET['s_val'];
		...
		$result=pg_query($conn,"select * from consumer_detail where cid=(select cid from order_detail where oid=$oid)") or die(pg_error($conn));
	...
	if(pg_query($conn,"update order_detail set status='$sval' where oid=$oid") or pg_error($conn))
