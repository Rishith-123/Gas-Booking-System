...
if(isset($_SESSION['did']))
{
	...
	$result=pg_query($conn,"select name from distributor_detail where did=$did");
	...
	//$dct=pg_fetch_row(pg_query($conn,"select city from distributor_detail where did=1"))[0] or die(pg_error($conn));
	
	//fetch all order whose status is not Delivered
	$result=pg_query($conn,"select o.oid,o.cid,c.name,o.date,o.time,o.amt,o.status from consumer_detail c
								INNER JOIN order_detail o
								ON c.cid=o.cid
								where o.did=$did and o.status!='Delivered'
								order by o.date,o.time ") or die(pg_error($conn));
