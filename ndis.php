...
if(isset($_REQUEST['dname']))
{
	...
	$result=pg_query($conn,"select * from distributor_detail where m_no='$dmno' or e_id='$demail'") or die(pg_error($conn));
	...
	else
	{
		...
		{			
			...
			$result=pg_query($conn,"select did from distributor_detail where m_no=$dmno") or die(pg_error($conn));
			...
			pg_query($conn,"update distributor_detail set proof='$target_file' where m_no=$dmno") or die(pg_error($conn));
			
			// For send back distibutor id 
			$q = "SELECT did FROM distributor_detail WHERE m_no=$dmno";
			$result=pg_query($conn,$q) or die(pg_error($conn));
			$r=pg_fetch_row($result);
