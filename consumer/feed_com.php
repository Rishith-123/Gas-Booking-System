...
if(isset($_SESSION['cid']))
{
	...
	$result=pg_query($conn,"select name from consumer_detail where cid=$cid") or die(pg_error($conn));
	...
	if((isset($_SESSION['cid'])) && isset($_POST['submit']))
	{
		...
		if($type=='Feedback')
		{
			...
			{
				$o_s=$_POST['o_s'];
				pg_query($conn,"insert into feedback_complain values($cid,(select did from consumer_detail where cid=$cid),CURRENT_DATE,CURRENT_TIME,'$type','$o_s','$msg')") or die(pg_error($conn));
			}
			else
			{
				pg_query($conn,"insert into feedback_complain values($cid,(select did from consumer_detail where cid=$cid),CURRENT_DATE,CURRENT_TIME,'$type','$f_sub','$msg')") or die(pg_error($conn));
			}
		}
		//for complaint
		else if($type==='Complaint')
		{
			...
			{	
				pg_query($conn,"insert into feedback_complain values($cid,(select did from consumer_detail where cid=$cid),CURRENT_DATE,CURRENT_TIME,'$type','$c_sub','$msg')") or die(pg_error($conn));				
			}
