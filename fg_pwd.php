...
if(isset($_GET['dis']))
{
	...
	if(isset($_POST['did']))
	{
		//for distributor pwd
		$did=$_POST['did'];
		$result=pg_query($conn,"select m_no,e_id,pwd from distributor_detail where did=$did") or die(pg_error($conn));
		...
		if(pg_num_rows($result)>0)
		{
			...
			echo "<script>alert('Details send to your registered email-id.')</script>";
		}
		else
		{
...
else if(isset($_GET['con']))
{
	...
	if(isset($_POST['cid']))
	{
		//for consumer pwd
		$cid=$_POST['cid'];
		$result=pg_query($conn,"select m_no,e_id,pwd from consumer_detail where cid=$cid") or die(pg_error($conn));
